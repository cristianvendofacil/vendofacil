...
export default function ListingEditorPage() {
  ...
  const loadAll = async () => {
    try {
      ...
      setMsg(e?.message || "Error cargando inmueble");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePhotoUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const path = `${userId}/${id}/${Date.now()}-${Math.random().toString(36).slice(2)}-${safeName}`;
  ...
  if (loading) {
    return (
      <main style={pageWrap}>
        <div style={container}>
          <div style={loadingCard}>Cargando inmueble...</div>
        </div>
      </main>
    );
  }
    ...
    <main style={pageWrap}>
      <div style={container}>
        ...
        <div style={{ marginTop: 18 }}>
          <div style={kicker}>🏠 Editor profesional de inmueble</div>
          <h1 style={titleStyle}>Editar inmueble</h1>
          <p style={subtitleStyle}>
            Completa los datos, elige una ciudad válida del sistema, sube fotos y selecciona el plan.
          </p>
        </div>
        ...
        <div style={layoutGrid}>
          <section style={card}>
            <div style={sectionTitle}>Datos del inmueble</div>
            ...
            <div style={formGrid}>
              <div style={fieldBlockFull}>
                ...
                <input
                  style={inputStyle}
                  placeholder="Ej: Casa amplia en Añelo"
                  value={title}
              ...
              <div style={fieldBlock}>
                ...
                <select style={inputStyle} value={town} onChange={(e) => setTown(e.target.value)}>
              ...
              <div style={fieldBlock}>
                ...
                <input style={{ ...inputStyle, background: "#F1F5F9", fontWeight: 800 }} value="ARS" disabled />
              </div>
              ...
              <div style={fieldBlockFull}>
                ...
                <textarea
                  ...
                  placeholder="Describe ubicación, comodidades, capacidad, servicios y todo lo importante."
                  value={description}
            ...
            <div style={uploadBox}>
              <div style={uploadTopRow}>
                ...
                />
                <div style={smallMuted}>Tu plan actual permite hasta {maxPhotosForPlan} fotos.</div>
              </div>

              {uploadingPhotos && <div style={{ marginTop: 12, fontWeight: 800 }}>Subiendo fotos...</div>}
                ...
                <div style={photoGrid}>
                  {photoUrls.map((url, index) => (
                    <div key={`${url}-${index}`} style={photoCard}>
                      ...
                      <button type="button" onClick={() => removePhoto(index)} style={removePhotoBtn}>
                        Eliminar
                      </button>
                    </div>
                  ))}
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                        <div style={{ fontWeight: 900, color: "#0F172A" }}>{config.label}</div>
                        <span
                          style={{
                      ...
                      <div style={{ marginTop: 8, color: "#64748B", fontSize: 14 }}>{config.note}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <section style={card}>
              ...
              <div style={summaryList}>
                <div style={summaryRow}>
                  <span>Operación</span>
                  <b>{listingType === "RENT" ? "Alquiler" : "Venta"}</b>
                </div>
              ...
              {!canPublish && <div style={errorBox}>Faltan datos para publicar: {validationErrors.join(", ")}</div>}

              <div style={actionGrid}>
                <button type="button" onClick={saveDraft} disabled={saving} style={saveBtn}>
                  {saving ? "Guardando..." : "Guardar borrador"}
                </button>

                <button type="button" onClick={continueToPayment} disabled={publishing} style={payBtn}>
